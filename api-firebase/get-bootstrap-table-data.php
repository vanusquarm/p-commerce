<?php
	/*  data of 'ORDERS' table  
		...
		data of 'SUBCATEGORY' table  
		data of 'PRODUCTS' table  
		data of 'USERS' aka 'Customers' table  
		data of 'Delivery Boy' table  
		data of 'Payment Request' table  
		data of 'Fund Transfer' table  
		data of 'Promo Codes' table  
		data of 'Return Request' table  
		data of 'Promo Codes' table  
		data of 'Wallet Transactions' table  
	*/
	session_start();
...
// 		print_r($res);
		...
		foreach($res as $row){
			...
			$tempRow['digital_address'] = $row['digital_address'];
...
// data of 'Promo Codes' table goes here
	...
	if(isset($_GET['table']) && $_GET['table'] == 'time-slots'){
		...
		$offset = 0; $limit = 10;
		$sort = 'last_order_time'; $order = 'ASC';
		$where = '';
		if(isset($_GET['offset']))
			$offset = $_GET['offset'];
		if(isset($_GET['limit']))
